...
def build_word_counts_for_chunks(
    ...
    for segment in segments:
        for match in compiled_pat.finditer(segment):
            ...
            word_counts[word_to_byte_tuple(pre_token)]+=1
    return word_counts


#作为多进程/多线程的适配器，因为map只能接受单参数函数，所以我们把多个参数打包成一个元组传递进去，然后在函数内部再解包。
def _build_word_counts_for_chunks(args):
...
def build_word_counts(
    ...
    if not special_tokens:
        raise ValueError("Special tokens list cannot be empty.")
    
    split_special_tokens=special_tokens[0].encode('utf-8')

    with open(input_path,'rb') as f:
        boundaries=find_chunk_boundaries(
            f,
            desired_num_chunks=num_processes,
            split_special_token=split_special_tokens
        )
        ...
        with mp.Pool(processes=num_processes) as pool:
            for partial_counts in tqdm(
                pool.imap_unordered(_build_word_counts_for_chunks, tasks),
                total=len(tasks),
                desc="Building word counts"
            ):
